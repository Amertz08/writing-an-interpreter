...
return 1;
	...
	for _, tt := range tests {
		...
		if errObj.Message != tt.expected {
			t.Fatalf("wrong error message. expected=%q, got=%q", tt.expected, errObj.Message)
		}
	}
}
...
func TestLetStatements(t *testing.T) {
	...
	for _, tt := range tests {
		evaluated := testEval(tt.input)
		if evaluated == nil {
			t.Fatalf("evaluated is nil. input=%q", tt.input)
		}
		testIntegerObject(t, evaluated, tt.expected)
	}
}
...
func TestFunctionObject(t *testing.T) {
	...
	if len(fn.Parameters) != 1 {
		t.Fatalf("function has wrong parameters. want 1, got=%d", len(fn.Parameters))
	}
...
false: 6,
	...
	for expectedKey, expectedValue := range expected {
		...
		if !ok {
			t.Fatalf("hash does not have the pair. key=%q", expectedKey)
		}
...
func testBooleanObject(t *testing.T, evaluated object.Object, expected bool) bool {
	...
	if result.Value != expected {
		t.Errorf("object.Value is %t, want %t", result.Value, expected)
		return false
	}
	return true
}
...
func testIntegerObject(t *testing.T, obj object.Object, expected int64) bool {
	...
	if result.Value != expected {
		t.Errorf("object.Value is %d, want %d", result.Value, expected)
		return false
	}
